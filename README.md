...
flowchart LR
    ...
    LEDGER --> PDF["Resi PDF\nadmin / investor"]
    LEDGER -.->|reversal| REV["Distribusi pembalik\nnominal dinegasikan"]
...
5. **Refund/reversal** → masih sebagai admin, buka distribusi `SETTLED` mana pun → **Balik distribusi ini**. Membuat distribusi pembalik (nominal negatif dengan sengaja) yang menegasikan setiap mutasi ledger tanpa menghapus riwayatnya — transaksi asal berubah jadi `REFUNDED`.
...
| ↩️ | Reversal, bukan edit | Distribusi pembalik baru (nominal negatif), ledger asli tak pernah disentuh — trigger database pun melarangnya | `/distribusi/:id` → "Balik distribusi ini" |
...
- **Kolom unik yang diam-diam memblokir reversal.** `transactionId` di tabel distribusi awalnya `@unique` penuh — secara struktural mustahil punya dua baris distribusi (asli + pembalik) untuk satu transaksi yang sama. Bukan soal kehabisan waktu; konflik skemanya baru kelihatan begitu ada yang benar-benar mencoba menulis kode reversal. Diperbaiki dengan *partial unique index* (`WHERE reversal_of_id IS NULL`).
